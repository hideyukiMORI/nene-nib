...
Vim は NORMAL / INSERT（`h j k l 0 $ w b e ^`・Home / End・回数はオペレータ側と移動側の掛け算・`x`・`d c y` ＋移動・`dd cc yy`・`D C Y`・`p P`・種類つきの無名レジスタ（本文は LF）・`i a I A`・Esc・`u` / Ctrl-r・INSERT 1 回が undo 1 単位・VISUAL `v V`）で、再現度は本物の Vim 9.1 の oracle が生成した fixture 256 件（`tests/vim/`・`eng/vim-oracle.py --regenerate`・生成物と json の一致は CNF-010）を CTest が再生して守る。IME は IMM32 を ui/win32 が受け、変換中の文字列は本文の外（`core::Composition`）に持って renderer がキャレットの行に差し込んで描き、確定は 1 意図（通常モードは undo 1 単位・Vim INSERT は打鍵として engine へ）。Vim NORMAL では IME を切り INSERT で戻す（ADR 0014）。core にテーマの模型と組み込み 9 テーマ（ADR 0017）があるが UI は OS の明暗で 2 テーマを選ぶだけ。`Ctrl-v`・画面の高さが要る移動・複数タブ・Ctrl+P・設定の保存・`:colorscheme` はまだ無い。起動は窓が約 35 ms で見え、本文は `D3D11CreateDevice`（NVIDIA のドライバ初期化 160 ms・呼び方では縮まない）の後に約 200 ms で出る（ADR 0013）。最新は [日報](docs/reports/2026-09-19.md) / [引き継ぎ](docs/handoffs/2026-09-19.md)。
