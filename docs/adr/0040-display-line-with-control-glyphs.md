...
決定 6 の「既知の差」を閉じた。ADR 0034 の表に `hex`（4 桁・`U+0080`〜`U+009F`）を足し、`display_line` は `hex` を `<` と小文字 16 進 2 桁と `>` に置き換える（`U+0085` → `<85>`・`U+009F` → `<9f>`）。`is_replaced` は真で、色は他の置き換えと同じ `muted`。書式用文字の `<200b>`（16 進 4 桁）と DEL（1 桁のまま）は変えない。
