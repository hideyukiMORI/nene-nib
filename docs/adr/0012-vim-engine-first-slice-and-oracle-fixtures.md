...
3. **`VimEffect` は閉じた `std::variant`**: `NoEffect` / `MoveTo{Offset}` / `RemoveRange{OffsetRange}` / `InsertString{std::string}` / `Undo` / `Redo`。`EditorController::accept(const VimKeyPress&)` が `std::visit` で既存の
...
10. **見た目は採用案のまま**（D15）: `mode_label` が NORMAL / INSERT、`CaretView::shape` が NORMAL でブロック・INSERT でバー。トグルで Vim に入ると NORMAL（回数・オペレータは空）、通常に戻ると保留は捨てる
...
正直に: `col('.')` はバイトなので多バイト文字でも `Offset` と一致するが、Vim の `virtcol` と表示上の列（全角・タブ）は別で、矩形（`Ctrl-v`）の縦切りで扱う。oracle は Vim 9.1 の版に依存し、版を上げると fixture の差分として現れる。
