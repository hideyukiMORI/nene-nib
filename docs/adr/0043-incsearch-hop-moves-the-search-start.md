...
2. **hop の意図**: application の意図の閉じた和型に `SearchHop { core::VimSearchDirection relative; }` を足す（`src/application/SearchHop.hpp`・1 ファイル 1 型・`EditorIntent` に 1 つ）。`accept(SearchHop)` は `SearchLine` が開いていて `incsearch` が on で `match` があるときだけ動く。**hop の向きは本文の順**（Ctrl-G は下へ・Ctrl-T は上へ）で、`/` `?` の検索方向に依らない（Sonnet の probe の実測 `?be<C-G>` が下へ動いたことと、Vim の実装の記憶が一致した。help の "next / previous match" を検索方向に相対と読む案は却下。hide の対話 Vim で確かめる項目）。手順: 今の当たり `match` から本文の順の向きに `vim_find_match(text, pattern, {match, 向き, count})` を引いて新しい当たり `M'` を置き、**起点 `from` は「確定の検索（検索方向・同じ回数）が `M'` に着く位置」**、すなわち `M'` から検索方向の逆へ `count` 件戻った当たりにする（Ctrl-G を `/` で押したときは今の当たりに等しい）。`count` は `[count]/pat` の回数で、preview の最初の検索も hop も確定と同じ回数を使う（#148 の preview は回数を見ていなかった。`2/be` の preview は 2 件目・文脈の (b)）。回数は engine の `vim_search_count` を application が読む（core の公開関数 1 本・確定と同じ値）。折り返しは `vim_find_match` の規則のまま（報せは出さない・ADR 0041 決定 3）。`match` が無い・`incsearch` off・Ex や palette の入力行では何もしない（文字を挿入しない。Vim は `noincsearch` だと `^G` を文字として入れるが、Nib は入力行に制御文字を入れない・ADR 0032）。
...
失うもの・残る穴: Ctrl-G / Ctrl-T は本物の Vim で観測できないので契約は help の読みに依る（`?` のときの Ctrl-G の向き（本文の順）・折り返し・回数の数え直しは hide の対話 Vim で確かめる）。search-offset は Nib に無い。`:s` `:g` への適用は後続。
...
| hop の向きを検索方向に相対にする（`?` の Ctrl-G は上へ） | help の文面からの読みで、probe の実測（`?be<C-G>` は下へ）と一致しない。実測に従う |
