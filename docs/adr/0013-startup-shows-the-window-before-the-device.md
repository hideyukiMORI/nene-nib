...
実測（2026-09-17・施主の実機・Release。`docs/quality/speed-reference.md`「窓を先に見せる」）: `startup-window-shown` は 5 回で 32.2〜38.0 ms・中央値 34.9 ms（忙しい機械の回。静かな回は 30.3〜32.5 ms）。基準値はこの 1 回目の値を採った（緩い側）。`startup-first-frame` は静かな回で 195.9〜201.6 ms（基準 191.5 ms・許容内）で、増分は `ShowWindow` の 15〜20 ms がゲートの区間に入ったぶん。窓が見えてから最初のフレームまでのクライアント中央の画素は 13 点すべて DWM のダーク Mica の面 (32,32,32) で、黒・白は無し（`eng/verify-window.py` の `verify_first_paint`）。却下の条件には当たらなかった。
正直に: この決定で速くなるのは体感だけで、`startup-first-frame` の数字は 5〜10 ms 遅くなった（許容内。基準値は変えない）。DWM が Mica を描くまでの時間（DWM 側の合成の遅れ）は `window_shown`（`ShowWindow` が返った直後）には入らず、本当に画素が光る時刻は測らない（ADR 0011 の決定 8 と同じ立場）。device の生成を worker に出して窓の生成やファイルの読み込みと重ねる案 B は、重ねられるのが最大 60 ms 程度で 159 ms は消えず、ADR 0004 の枠（worker はポートの実装）に device を置く形が要るので、この ADR では採らない（16 MiB の読み込みが重くなったら再検討）。
...
| 窓が見えてから本文が出るまでの 160 ms にロゴ・プログレスバー・ローディングの表示を出す（施主の提案 2026-09-17） | 160 ms は表示を出す長さではない（100 ms 以下は即時、1 秒までは間があるだけ、進捗表示が効くのは 1 秒超）。出て消える表示がそれ自体チラつきになる。描くには D2D の前に別の描画経路かタイマーが要る（ADR 0007 / 0011）。施主の所見は「チラつく感じではない」。遅い機械で 500 ms を越える環境が出たら、その機械で次回起動から出す設計を ADR で足す |
