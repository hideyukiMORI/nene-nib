...
CI（GitHub Actions・`windows-2022`）でも窓が作れてベンチが動く。基準値は実機と同じく**指紋ごと**に持ち、いま持っているのは `e7a87d5b6ac1e14b`（AMD EPYC 7763）の 1 つだけ（[ADR 0016](../adr/0016-ci-speed-reference-per-host-fingerprint.md)・Issue #47）。
指紋の一致しない host は記録だけで終了 0 だが、`Speed: no reference for <指紋> (<CPU>); recorded only -- QLT-014 is not judged on this host` を出して黙らない（`eng/check.ps1` はこの行をまとめにも出す）。
ダッシュを ASCII の `--` で書くのは、U+2014 が cp932 に無く（0x815C は U+2015）、施主の机で `print` が `UnicodeEncodeError` になってゲートが「退行」と言ってしまうから（2026-09-18・実測。[ADR 0016](../adr/0016-ci-speed-reference-per-host-fingerprint.md) の決定 3 の文言そのもの）。窓が作れなければ `window unavailable` を記録して終了 0。
...
**実機（RTX 3090）との差は起動と 16 MiB のどちらも約 170 ms。** ファイルの読み込みではなく、実機の最初のフレームに GPU / DWM 側の固定費が乗っていると読める。内訳は「起動の内訳（Issue #19）」の節で測った（`device_created` に 159.6 ms）。CI の基準値は同じ CI 機の値が数回たまってから別の Issue で決める（ADR 0006 の決定 1・ADR 0011 の決定 4）→ **Issue #47 で決めた**（下）。

## CI の記録（Issue #47・ADR 0016・調査 2026-09-18）

CI の記録（`out/speed/*.json`）は `.gitignore` の中で、artifact も無かったので 1 つも残っていない。
下の値は `gh run view <id> --log` の `Speed:` の行から拾った（だから小数 3 桁）。単位は ms、`med` は 1 run の 5 試行の中央値、括弧は 5 試行の最小／最大。欠測はどの run も 0。

| run | 日付 | PR | 指紋 | CPU | first-frame med (min/max) | window-shown med (min/max) | key-single med (min/max) | key-burst-200 med (min/max) | 16MiB med (min/max) |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 35002057290 | 09-15 | #18 | `e7a87d5b` | AMD EPYC 7763 | 20.428 (19.962/258.866) | —（未実装） | 1.344 (1.304/1.995) | 2.700 (2.564/7.982) | 70.085 (69.901/75.613) |
| 35003147069 | 09-15 | #21 | `9279629d` | Intel Xeon 6973P-C | 18.407 (16.128/311.013) | —（未実装） | 1.255 (1.237/2.156) | 2.006 (1.949/8.052) | 68.329 (66.741/76.909) |
| 35104611209 | 09-16 | #23 | `e7a87d5b` | AMD EPYC 7763 | 21.500 (19.839/303.507) | —（未実装） | 1.451 (1.440/2.058) | 2.992 (2.866/8.854) | 84.122 (83.117/88.050) |
| 35105144576 | 09-16 | #23 | `169363bd` | AMD EPYC 9V74 | 16.530 (16.367/240.349) | —（未実装） | 1.159 (1.136/1.659) | 2.161 (2.160/7.348) | 70.153 (69.699/77.202) |
| 35113650997 | 09-16 | #25 | `e7a87d5b` | AMD EPYC 7763 | 20.133 (19.626/1485.053) | —（未実装） | 1.454 (1.262/44.572) | 2.570 (2.498/19.054) | 70.326 (69.751/82.495) |
| 35115189778 | 09-16 | #27 | `56763691` | Intel Xeon Platinum 8370C | 21.929 (21.438/302.388) | —（未実装） | 1.436 (1.409/50.445) | 3.073 (2.975/67.511) | 80.081 (77.860/85.322) |
| 35121845572 | 09-16 | #29 | `e7a87d5b` | AMD EPYC 7763 | 37.023 (35.117/293.610) | 26.480 (25.532/34.392) | 1.446 (1.363/6.514) | 2.886 (2.726/7.988) | 108.886 (101.291/121.488) |
| 35136242623 | 09-16 | #34 | `196309bb` | AMD EPYC 9V45 | 25.890 (23.454/1426.572) | 18.763 (16.338/115.988) | 1.153 (1.101/1.561) | 2.102 (1.902/32.985) | 61.870 (58.213/203.741) |
| 35208906192 | 09-17 | #32 | `e7a87d5b` | AMD EPYC 7763 | 37.011 (34.927/343.493) | 26.788 (25.303/29.525) | 1.514 (1.326/2.036) | 2.832 (2.718/12.495) | 88.709 (87.477/89.319) |
| 35209555298 | 09-17 | #32 | `e7a87d5b` | AMD EPYC 7763 | 37.986 (36.536/276.891) | 27.651 (27.348/28.290) | 1.293 (1.268/12.759) | 2.670 (2.529/74.943) | 87.002 (85.481/97.225) |
| 35212506015 | 09-17 | #35 | `be35b5ff` | Intel Xeon Platinum 8573C | 33.912 (33.461/488.632) | 25.742 (25.061/29.980) | 1.171 (1.122/1.740) | 2.205 (2.125/18.129) | 87.826 (85.969/95.994) |
| 35216632797 | 09-17 | #37 | `e7a87d5b` | AMD EPYC 7763 | 36.516 (35.726/295.140) | 26.819 (25.689/31.177) | 1.396 (1.366/2.153) | 2.679 (2.638/14.971) | 86.768 (86.054/95.458) |
| 35217895642 | 09-17 | #40 | `e7a87d5b` | AMD EPYC 7763 | 36.282 (33.676/354.692) | 26.931 (24.600/29.014) | 1.392 (1.351/2.068) | 2.947 (2.716/9.104) | 89.262 (86.317/94.863) |
| 35219937347 | 09-17 | #42 | `e7a87d5b` | AMD EPYC 7763 | 34.403 (32.985/300.022) | 24.825 (23.642/27.000) | 1.389 (1.220/2.056) | 2.620 (2.398/8.166) | 85.585 (84.750/92.072) |
| 35353389292 | 09-18 | #45 | `169363bd` | AMD EPYC 9V74 | 26.770 (26.677/654.142) | 19.104 (19.059/21.323) | 1.152 (1.135/56.494) | 2.200 (2.181/13.056) | 67.155 (66.335/68.500) |
| 35354289264 | 09-18 | #46 | `e7a87d5b` | AMD EPYC 7763 | 36.349 (35.710/818.261) | 26.788 (26.346/27.751) | 1.477 (1.407/2.052) | 2.683 (2.621/26.075) | 87.349 (85.195/91.549) |

### 指紋の分布

GPU は 16 run とも `Microsoft Hyper-V Video`（WARP）・96 DPI で、**違うのは CPU 名だけ**。ランナーの host は 6 種類あった。

| 指紋 | CPU | run 数 |
| --- | --- | --- |
| `e7a87d5b6ac1e14b` | AMD EPYC 7763 64-Core Processor | 10 |
| `169363bddab9a7fd` | AMD EPYC 9V74 80-Core Processor | 2 |
| `9279629dcefaae67` | Intel(R) Xeon(R) 6973P-C | 1 |
| `567636915fb3c070` | Intel(R) Xeon(R) Platinum 8370C CPU @ 2.80GHz | 1 |
| `196309bbf27c16b1` | AMD EPYC 9V45 96-Core Processor | 1 |
| `be35b5ff6525a0fc` | INTEL(R) XEON(R) PLATINUM 8573C | 1 |

**CPU 世代の差は許容 25% を越える**（#24 以降で startup-first-frame 25.9 → 38.0 ms、16 MiB 61.9 → 108.9 ms）。
速い host の値を基準にすると遅い host の run が偽の退行で落ち、逆なら速い host の退行を見逃す。だから指紋を鍵にしたまま項を足す（ADR 0016 の決定 1）。

### #24 の前後で startup の意味が変わった

`startup-window-shown` は #24（[ADR 0013](../adr/0013-startup-shows-the-window-before-the-device.md)）で足した鍵なので、それより前の 6 run には無い（表の「未実装」）。
同じ #24 で窓を device より先に見せるようにしたため、`startup-first-frame` も 20 ms 前後 → 36 ms 前後、16 MiB も 70 ms 前後 → 87 ms 前後へ動いた。
**#24 より前の 6 run は基準値の材料にならない。**

### 採用した基準値（`e7a87d5b6ac1e14b`）

材料は **#24 以降・同じ指紋の 7 run**（35121845572 / 35208906192 / 35209555298 / 35216632797 / 35217895642 / 35219937347 / 35354289264）。
`medianMs` は run ごとの中央値の**中央値**、`minimumMs` / `maximumMs` はその 7 つの中央値の最小・最大（1 run の 5 試行の幅ではない）。
1 run の `--adopt` にしないのは、CI の 1 試行目がコールドスタートの外れ値（240〜1485 ms）を含み、その run の荒れがそのまま基準になるから（ADR 0016 の決定 2）。

| ベンチ | 中央値の中央値 | 7 つの中央値の最小 | 最大 | 最大/最小 | 25% ＋ floor 2 ms の上限 | 最遅 run は落ちるか |
| --- | --- | --- | --- | --- | --- | --- |
| startup-first-frame | 36.516 | 34.403 | 37.986 | 1.10 | 45.645 | 通る |
| startup-window-shown | 26.788 | 24.825 | 27.651 | 1.11 | 33.485 | 通る |
| key-to-frame-single | 1.396 | 1.293 | 1.514 | 1.17 | 1.745 | 通る |
| key-to-frame-burst-200 | 2.683 | 2.620 | 2.947 | 1.12 | 3.354 | 通る |
| open-large-file-16mib | 87.349 | 85.585 | 108.886 | 1.27 | 109.186 | 通る |

- **同じ指紋の中の揺れは 1.10〜1.27 倍**で、共通の許容（25% ＋ floor 2 ms）に収まる。7 run のうち最も遅い run も落ちない
- 16 MiB だけは #29 の 108.886 ms が上限 109.186 ms すれすれなので、机の荒れで落ちる回がありうる。**落ちたら run をやり直す**。基準値を上げるのは ADR の判断（ADR 0011 の決定 5）
- 打鍵 2 本は絶対値が小さく（1.2〜3.0 ms）、floor 2 ms に飲まれて 1.75〜2.4 倍まで落ちない。**CI では判定にならない（planned）**
- CI で退行として見えるのは `window_shown` の区間（この 7 run で 14.5〜16.5 ms）と 16 MiB の `document_opened`（47.4〜64.3 ms）で、実機の主費用 `device_created`（NVIDIA 160 ms）は CI では 3.7〜4.4 ms

### CI の起動の内訳（run 35354289264・中央値 ms）

| ベンチ | origin | document_opened | window_created | backdrop_applied | window_shown | device_created | swap_chain | composition | context | text_formats | frame_presented |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| startup-first-frame | 9.7 | 0.0 | 1.2 | 0.7 | 14.9 | 4.0 | 0.4 | 0.4 | 0.4 | 0.6 | 3.8 |
| open-large-file-16mib | 9.4 | 47.5 | 1.5 | 0.5 | 16.4 | 4.0 | 0.4 | 0.4 | 0.4 | 0.6 | 6.0 |

実機（RTX 3090・ADR 0013 の段 2 の列）と形が違うのは `device_created` で、実機 166.0 ms に対し CI は 4.0 ms（WARP にはドライバの初期化が無い）。
`window_shown` は実機 16.9 ms・CI 14.9 ms、`origin` は実機 12.4 ms・CI 9.7 ms でほぼ同じ形。
**CI の判定は CPU 側（`origin` / `window_shown` / `document_opened`）の退行を見張るもので、GPU 側の固定費は映さない。**

### 記録の残し方

`check.yml` が `out/speed/*.json` を `actions/upload-artifact`（名前 `speed-records`・保持 90 日・`if: always()`）で上げる。失敗した run の記録こそ要るので `always()`。
他の指紋の基準値をためる道はこれだけで、採用は人が artifact を取って複数 run の中央値の中央値を手で書く（CI で `--adopt` して commit はしない。ADR 0011 の決定 5・ADR 0016 の決定 4）。
