...
開いている Issue は無い。次の 3 候補の順を hide に聞く: (a) **Vim の 4 本目**（画面の高さが要る移動・`Ctrl-v`・ドラッグで VISUAL）(b) **C2**（設定の保存・D14 フォントサイズ）(c) **C3**（`:colorscheme`。Ex の `:` が先）。設計リナの推しは a → b → c。
Issue #1 / #3 / #5 / #7 / #9 / #11 / #13 / #14 / #16 / #19 / #22 / #24 / #28 / #30 / #31 / #36 / #43 / #44 / #47 / #52 / #53 は main へ統合済み（[日報 09-19](../reports/2026-09-19.md)）。main は `a322323`。
...
hide に順を聞いてから Issue を切る。**Vim の 4 本目**は oracle の `-es` で `set lines=N` が効くかの実測から（画面の高さを engine に渡す形・`VimScrollTo` の要否・`virtcol`）。**C2** は設定の置き場・形式・版を hide に聞いてから ADR。**C3** は Ex の `:` が先で、導出した 7 テーマの見た目を `build_boards.py` の絵にして hide が見る。順は引き継ぎ書。
