...
**最新の変更は Issue #68（C4a・利用者テーマの形式/所有/読込）**。厳密な版付きテーマファイルを既存Themeへ変換する基盤を追加した [ADR 0024](../adr/0024-user-theme-file-and-owned-values.md)。テーマ127 / 設定codec23 / core所有値21 checksが成功。アプリからの選択は次のC4b。統合状態はGitHubを正とする。
Issue #66 / PR #67（C3b）、#64 / PR #65（C3a）、#60 / PR #63（C2）、#58 / PR #59（Vim画面移動）、#61 / PR #62（差分検証・成功再利用）はmainへ統合済み。
...
**C4b**: C4aのThemeDocumentを不変のカタログとして保持し、設定保存/復元とEx・Ctrl+P候補へ接続する。壊れたテーマは名前付きで拒否し、既存の読込失敗後の保存禁止を保つ。Ctrl+Pのファイル・履歴統合、矩形選択とドラッグは別Issue。
