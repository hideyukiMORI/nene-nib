...
| 項目 | 値 | 正本 |
| --- | --- | --- |
| Vim fixture | 1423 件（`macro-*` 20 件は `register` 欄で再生だけ・`register-*` 25 件・`space-*`） | `tests/vim/VimFixtures.hpp` の 5 行目（CNF-010） |
| 既定の `nib_tests` | 14602 checks・scope 22（1 scope = 1 翻訳単位・表は `NibTests.cpp`・ADR 0042） | [gate-proofs 5-aw](../quality/gate-proofs.md)。前後比較は `python eng/protected-diff.py --base <ref> --build`（#130・`out/protected/<短い SHA>.json`） |
| ADR | 0049 まで | [`docs/adr/README.md`](../adr/README.md) |
...
autoindent・数字・クリップボード・読み取り専用のレジスタ（`"0` `"+` `"*` `"-` `".`）・矩形レジスタへの `"A` の追記・`whichwrap` の設定・マクロの中の Ex と録画中の表示・`J s S R`・r の制御文字・Ctrl-e/y・検索の `:s` `:g`・履歴・offset・`\v` `\c` `\(` `\|` `\{`・`ignorecase`・
