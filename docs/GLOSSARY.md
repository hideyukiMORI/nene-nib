...
| Vimの次キー待ち | 文字検索の対象待ち、またはg/rの続き待ちの一方だけを持つ任意の和型 | `VimInputWait` / `VimPrefix`、所有は `VimState`（core・ADR 0027 / 0029） |
...
| 改行の形 | CRLF / LF の閉じた 2 つ。読んだときに本文が 1 度だけ判別して持ち回り、編集では変わらない。`\r` が改行の一部かどうかはこの形だけで決まる | `LineEnding` / `detect_line_ending`、所有は `TextBuffer`（core・ADR 0010 / 0036） |
| literal CR | LF 文書の中の `\r`。改行ではなく 1 文字で、桁を 1 つ数え（仮想桁は 2）、保存でもレジスタでも残る。Vim の `unix` の `^M` と同じ | `TextBuffer::line_end` の分岐（core・ADR 0036） |
...
| VISUALの範囲の大きさ | VISUALで行った変更を `.` が繰り返すために覚える大きさ。文字単位は行数と「行末まで/桁」、行単位は行数だけ。桁は仮想桁 | `VimVisualExtent` ＝ `VimCharacterExtent` / `VimLineExtent`（core・ADR 0033 / 0034） |
| 表示幅 | 1 つの code point が画面で占める桁の数。0 / 1 / 2 / 6（Vim が `<200b>` と描くもの）の閉じた 4 種類で、範囲表は固定 Vim 9.1 の `strdisplaywidth()` の全 code point 実測。ロケールにも描画にも依らない | `DisplayWidth` / `display_width` / `DisplayWidthRange`（core・ADR 0034） |
| 仮想桁 | 行頭からの表示幅で数える桁（Vim の virtcol・1 始まり・`tabstop=8` 固定）。code point で数える `Column` とは別の型で、欲しい列と VISUAL の `.` の桁だけが通る。Vim がキャレットを描く桁は Tab の上だけ最後の桁 | `VirtualColumn` / `virtual_column` / `caret_virtual_column` / `offset_at_virtual_column`（core・ADR 0034） |
