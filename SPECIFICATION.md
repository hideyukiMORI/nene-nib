...
| D11 | ダークの配色は Ubuntu をイメージした深い茄子色（施主決定 2026-09-15） | 背景は Ubuntu 端末の #300A24、文字は淡い灰 #EEEEEC を初期値にする。ライトは中立の明るい灰（#F4F5F7 / #1B1F24）。アクセント色（Ubuntu の橙 #E95420 など）は後の縦切りで決める |
...
| FR-004 | トグル: 1 キー＋ステータスバーのボタン。テキスト・undo・カーソルは共通のまま、キー割り当てとマウス規則をセットで入れ替える | D10。鍵は🔶決め打ちしない |
| FR-005 | タブはタイトルバーに横並び。Ctrl+Tab は最近使った順 | D6 |
